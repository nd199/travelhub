...
export default function ExpandDetails({ flight }) {
  const getAmenityIcon = (amenity) => {
    switch (amenity) {
      ...
      case 'Entertainment':
        return <FaTv className="w-4 h-4 text-blue-600" />;
      case 'Extra Baggage':
        return <FaSuitcase className="w-4 h-4 text-purple-600" />;
      default:
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="space-y-2">
          {Object.entries(flight.price).map(([className, price]) => (
            <div key={className} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-gray-500">
                  ({flight.seats[className] || 0} seats available)
                </span>
              </div>
      ...
      {/* Flight Information */}
      <div>
        <h4 className="text-sm font-semibold text-gray-900 mb-3">Flight Information</h4>
        <div className="grid grid-cols-2 gap-3">
          <div className="flex items-center gap-2 text-sm">
            <FaPlane className="w-4 h-4 text-gray-500" />
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-gray-900">{flight.type}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(flight.amenities).map(([amenity, available]) => (
            available && (
              <div
                key={amenity}
      ...
      <div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-gray-900">{flight.boardingPoint}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-600 w-20">To:</span>
            <span className="font-medium text-gray-900">{flight.droppingPoint}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
